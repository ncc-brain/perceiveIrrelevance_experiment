function[full_output]= mcnemar(x,varargin)
...
%save the variable to return it (added later to the original script)
output1 = ['Critical value at ' num2str((1-alpha)*100) '% significance level = ' num2str(chi2inv(1-alpha, 1))];
output2 = sprintf('McNemar chi-square (with Yates'' correction) = %0.6f    p = %0.6f', chisquare, 1 - chi2cdf(chisquare, 1));
output3 = sprintf('alpha = %0.4f  Zb = %0.4f  Power (2-tails) = %0.4f', alpha, Zb, pwr);

% Concatenate into one variable
full_output = sprintf('%s\n%s\n%s', output1, output2, output3);
